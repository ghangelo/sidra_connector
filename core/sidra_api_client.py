...
class SidraApiClient:
    ...
    def fetch_and_parse(self, params: dict = None) -> tuple:
        ...
        if 'text/html' in content_type or normalized_text.startswith('<!doctype html') or normalized_text.startswith('<html') or any(marker in normalized_text for marker in challenge_markers):
            raise ValueError(f"O endpoint do SIDRA respondeu com página de bloqueio: {final_url}")
        ...
        try:
            ...
            raise ValueError(f"Resposta da API não é JSON válido: {final_url}") from exc
